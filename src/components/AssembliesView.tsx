...
export default function AssembliesView() {
  const { assemblies, assemblyVotes, addAssembly, updateAssembly, voteAssembly, professionals, activeTenantId, user } = useHomeCareStore();
  ...
  const resetForm = () => { setTitle(''); setDescription(''); setDocumentUrl(''); };
  const openEditor = (assembly: typeof assemblies[number]) => {
    setEditingAssemblyId(assembly.id); setTitle(assembly.title); setDescription(assembly.description);
    setDocumentUrl(assembly.documentUrl || ''); setShowAddModal(true);
  };
  ...
  const handleCreateAssembly = (e: React.FormEvent) => {
    ...
    const assemblyData = {
      title,
      description,
      status: 'active' as Assembly['status'],
      date: new Date().toISOString().split('T')[0],
      documentUrl
    };
    if (editingAssemblyId) updateAssembly(editingAssemblyId, assemblyData);
    else addAssembly(assemblyData);

    resetForm(); setEditingAssemblyId(null);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          onClick={() => { resetForm(); setEditingAssemblyId(null); setShowAddModal(true); }}
            ...
            <div key={assembly.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
              <div className="p-5 border-b border-slate-100 flex-1 space-y-3">
                <div className="flex justify-between items-start">
                  ...
                  <button onClick={() => openEditor(assembly)} className="p-1 text-slate-400 hover:text-indigo-600 rounded" title="Editar assembleia"><Pencil className="w-4 h-4" /></button>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-30 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full border border-slate-200 shadow-2xl">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-base text-slate-800">{editingAssemblyId ? 'Editar Assembleia' : 'Nova Assembleia'}</h3>
              </div>
            </div>
            ...
            <form onSubmit={handleCreateAssembly} className="p-6 space-y-4">
              ...
              <div className="border-t border-slate-100 pt-5 flex justify-end gap-3 mt-6">
                <button
                  type="button"
                  onClick={() => { resetForm(); setEditingAssemblyId(null); setShowAddModal(false); }}
                ...
                >
                  <span className="flex items-center gap-1.5"><Save className="w-3.5 h-3.5" />{editingAssemblyId ? 'Salvar Alterações' : 'Publicar Assembleia'}</span>
                </button>
              </div>
            </form>
          </div>
